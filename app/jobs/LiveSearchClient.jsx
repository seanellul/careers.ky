...
const LOCATION_KEY = {
  ...
  6: "Red Bay / Prospect",
  7: "Spotts / Newlands",
  8: "Savannah / Lower Valley",
  9: "Bodden Town",
  10: "North Side",
  11: "East End",
  12: "Rum Point / Cayman Kai",
  13: "Cayman Brac",
  14: "Little Cayman",
};
const WORK_TYPE = {
...
export default function LiveSearchClient({
  ...
  const stats = {
    ...
    avgSalary:
      filtered.reduce((acc, j) => acc + (j.maximumAmount || j.minimumAmount || 0), 0) /
    ...
    <div className={embedded ? "" : "mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 md:py-12"}>
      <div className="mb-12">
        ...
        <p className="text-neutral-600 dark:text-neutral-400 text-lg max-w-3xl">
          Browse {stats.total} active job postings across {stats.industries}+ industries in Cayman.
          Real-time data from WORC.
        </p>
        ...
        <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
          <CardContent className="p-4 md:p-6">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl bg-primary-50 dark:bg-primary-500/15 grid place-items-center">
                <TrendingUp className="w-5 h-5 text-primary-500" />
              </div>
              <div>
                <div className="text-xl sm:text-2xl md:text-3xl font-semibold">{stats.total}</div>
                <div className="text-xs md:text-sm text-neutral-500">Active Jobs</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4 md:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs md:text-sm text-neutral-500">Industries</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4 md:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs md:text-sm text-neutral-500">Locations</div>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xl md:text-2xl font-semibold truncate">
                  CI$ {Math.round(stats.avgSalary / 1000)}k
                </div>
                <div className="text-xs md:text-sm text-neutral-500">Avg Salary</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="pl-10 bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700 h-12 text-base"
              />
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      value={occGroup}
                      onChange={(e) => {
                        setOccGroup(e.target.value);
                        setPage(1);
                      }}
                      className="w-full bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 rounded-xl px-3 py-2 h-10 text-sm text-neutral-700 dark:text-neutral-300"
                    >
                      <option value="">All Occupations</option>
                      {occGroupOptions.map((o) => (
                  ...
                  {selectedSkills.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {selectedSkills.map((s) => (
                        <Badge
                          key={s.id}
                          className="bg-primary-50 dark:bg-primary-500/15 text-primary-700 dark:text-primary-300 border-primary-200 dark:border-primary-500/30 pr-1 flex items-center gap-1"
                        >
                          {s.name}
                          <button
      ...
      <div className="mb-6 text-sm text-neutral-600 dark:text-neutral-400">
        ...
        {Math.min(page * pageSize, sorted.length)} of {sorted.length} jobs
      </div>
      ...
      >
        {paginating
          ? Array.from({ length: pageSize }).map((_, i) => (
              <Card
                key={i}
                className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700 h-full"
              >
          ...
          : view.map((j, idx) => {
              ...
              return (
                <Card
                  key={`${j.jobPostId || idx}`}
                  className="job-card group bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700 hover:border-neutral-300 dark:hover:border-neutral-600 hover:shadow-lg hover:shadow-primary-200/10 hover:-translate-y-0.5 transition-all duration-200 h-full"
                >
                  <CardContent className="p-5 h-full flex flex-col">
                    <div className="mb-2">
                      <span className="inline-block text-xs font-medium uppercase tracking-wide text-primary-700 dark:text-primary-300 bg-primary-50 dark:bg-primary-500/15 border border-primary-200 dark:border-primary-500/30 rounded-full px-2.5 py-0.5">
                        {WORK_TYPE[j.workType] || wtObj[j.workType] || "Role"}
                      </span>
                    </div>
                    <Link
                      href={`/jobs/${j.jobPostIdString || j.jobPostId}`}
                      className="font-medium leading-tight group-hover:text-primary-500 mb-1.5 line-clamp-2 min-h-[2.5rem] flex items-start transition"
                    >
                    ...
                    <div className="text-sm text-neutral-500 mb-1.5 line-clamp-1">
                      ...
                      >
                        {titleCase(j.employerName)}
                      </Link>
                    </div>
                    <div className="flex items-center gap-1 text-xs text-neutral-500 mb-3">
                      <MapPin className="w-3 h-3 shrink-0" />
                      <span>
                        {LOCATION_KEY[j.jobLocation] || ltObj[j.jobLocation] || "Cayman Islands"}
                        {j.hoursPerWeek ? ` · ${j.hoursPerWeek} hrs/wk` : ""}
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5 mb-3">
                      ...
                      {salary.suffix && (
                        <span className="text-xs text-neutral-500">{salary.suffix}</span>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {etObj[j.educationLevel] && (
                        <Badge className="bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 border-neutral-200 dark:border-neutral-700 text-xs px-2 py-0.5 gap-1">
                          ...
                          {truncateText(etObj[j.educationLevel], 22)}
                        </Badge>
                      )}
                      {exObj[j.yearsOfExperience] && (
                        <Badge className="bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 border-neutral-200 dark:border-neutral-700 text-xs px-2 py-0.5 gap-1">
                          <Clock className="w-3 h-3" />
                          {truncateText(exObj[j.yearsOfExperience], 22)}
                        </Badge>
                      )}
                    </div>
                              ...
                              <>
                                {isCandidate && !alreadySent && (
                                  ...
                                  >
                                    ...
                                    {sendingInterest === jobId ? "Sending..." : "Express Interest"}
                                  </button>
                                )}
                                {isCandidate && alreadySent && (
                                ...
                                {!session && (
                                  <Link
                                    href="/profile/setup"
                                    className="inline-flex items-center justify-center whitespace-nowrap rounded-xl text-sm font-medium bg-primary-50 dark:bg-primary-500/15 text-primary-500 border border-primary-200 dark:border-primary-500/30 hover:bg-primary-100 w-full gap-2 h-10 px-4 py-2 transition"
                                  >
                                    <HeartHandshake className="w-4 h-4" /> Sign in to Express
                                    Interest
                                  </Link>
                                )}
                              </>
