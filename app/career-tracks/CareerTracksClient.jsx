...
export default function CareerTracksClient({
  ...
  useEffect(() => {
    resetPage();
  }, [filters, sortBy, sortOrder, selectedCiscoCodes, resetPage]);
  ...
  const Bar = ({ label, value, total, color = "bg-primary-300" }) => (
    <div className="flex items-center gap-2">
      <div
        className="w-16 sm:w-32 text-xs text-neutral-600 dark:text-neutral-400 truncate"
        title={label}
      >
        {label}
      </div>
    ...
    <div className={embedded ? "" : "mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8"}>
      <div className="mb-8">
        ...
        <p className="text-neutral-600 dark:text-neutral-400 text-lg max-w-3xl">
          Discover the complete job market in Cayman through our comprehensive CISCO taxonomy. Find
          roles by industry, salary ranges, and requirements.
        </p>
      </div>
                  ...
                  return (
                    ...
                    >
                      <div className="font-medium flex items-center justify-between">
                        ...
                        {isSelected && (
                          <Badge className="bg-primary-500 text-white text-xs">✓</Badge>
                        )}
                      </div>
            ...
            <CardContent className="p-4">
              <div className="flex flex-wrap items-center gap-4 mb-4">
                ...
                <select
                  ...
                  onChange={(e) => setFilters((p) => ({ ...p, education: e.target.value }))}
                  className="bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 rounded-lg px-3 py-2 h-10 text-sm"
                >
              ...
              {(tree.children || []).map((major) => (
                ...
                >
                  ...
                  <div className="text-sm text-neutral-500">
                    {major.children?.length || 0} subcategories
                  </div>
                </button>
              ))}
            </div>
              ...
              <div className="space-y-2">
                {selectedMajor.children?.map((subMajor) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-neutral-500">
                      {subMajor.children?.length || 0} specializations
                    </div>
                  </button>
                ))}
              </div>
                        ...
                        <CardContent className="p-3">
                          ...
                          {stats && (
                            <div className="flex gap-2 text-xs">
                              <Badge className="bg-white dark:bg-neutral-800 shadow-sm">
                                {stats.count} posts
                              </Badge>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-base mb-1 group-hover:text-primary-500 transition line-clamp-2">
                          {unit.title.replace(/\s+not elsewhere classified$/i, "")}
                        </h3>
                        <p className="text-xs text-neutral-500 line-clamp-2">{unit.description}</p>
                      </div>
                      <Badge className="bg-primary-50 dark:bg-primary-500/15 text-primary-500 border-primary-200 dark:border-primary-500/30 ml-2 shrink-0">
                        {unit.id}
                      </Badge>
                    </div>
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <div className="space-y-1">
                              {Object.entries(stats.dist.work)
                                .filter(([, v]) => v > 0)
                                .sort(([, a], [, b]) => b - a)
                                .slice(0, 3)
                                .map(([k, v]) => (
