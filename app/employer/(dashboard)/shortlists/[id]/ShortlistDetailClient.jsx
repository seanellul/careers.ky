      ...
      <div className="mb-6">
        <Link href="/employer/shortlists">
          <Button variant="secondary" className="gap-2">
            <ChevronLeft className="w-4 h-4" /> Back to Shortlists
          </Button>
        </Link>
      </div>
      ...
      {candidates.length > 0 ? (
        <div className="space-y-3">
          {candidates.map((c, i) => (
            <Card
              key={c.id}
              className={`bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700 ${selectedCandidates.has(c.candidate_id) ? "ring-1 ring-primary-300 dark:ring-primary-500/40" : ""}`}
            >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <div className="w-8 h-8 rounded-full bg-neutral-100 dark:bg-neutral-800 grid place-items-center">
                      ...
                      {introSent.has(c.candidate_id) && (
                        <Badge className="bg-emerald-500/20 text-emerald-300 border-emerald-300/30">
                          <CheckCircle className="w-3 h-3 mr-1" /> Intro Sent
                        </Badge>
                      )}
                    </div>
                    <div className="grid grid-cols-3 gap-2 text-sm mb-2">
                      <div className="flex items-center gap-1">
                        ...
                        {eduTypes.get(c.education_code) || "Not set"}
                      </div>
                      <div className="flex items-center gap-1">
                        <Clock className="w-3 h-3 text-orange-300" />{" "}
                        {expTypes.get(c.experience_code) || "Not set"}
                      </div>
                      <div className="flex items-center gap-1">
                        <MapPin className="w-3 h-3 text-pink-300" />{" "}
                        {locTypes.get(c.location_code) || "Not set"}
                      </div>
                    </div>
                    ...
                    {c.notes && (
                      <div className="mt-2 text-xs text-neutral-500 italic">Notes: {c.notes}</div>
                    )}
                  </div>
                  <button
