...
function buildPrintDocument(report, employerName) {
  ...
  const pipelineRows = Object.entries(summary.stageBreakdown || {})
    .map(
      ([stage, count]) =>
  ...
  const rejectionRows = Object.entries(summary.rejectionBreakdown || {})
    .map(
      ([reason, count]) =>
...
<body>
  ...
  ${
    intros.length === 0
      ? "<p>No candidates were contacted for this position.</p>"
      : `
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <Link
          ...
          className="flex items-center gap-2 text-neutral-500 hover:text-neutral-900 dark:hover:text-neutral-100 transition text-sm"
        >
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-neutral-500 space-y-0.5">
                ...
                <div>
                  <strong>WORC Job ID:</strong> {job.job_id}
                </div>
                <div>
                  <strong>Posted:</strong> {formatDate(job.created_date)} &mdash;{" "}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
          <CardContent className="p-4 text-center">
            <Users className="w-5 h-5 mx-auto mb-1 text-primary-500" />
            <div className="text-2xl font-semibold">{summary.totalContacted}</div>
            <div className="text-xs text-neutral-500">Total Contacted</div>
          </CardContent>
        </Card>
        <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
          <CardContent className="p-4 text-center">
        ...
        <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
          <CardContent className="p-4 text-center">
            <Shield className="w-5 h-5 mx-auto mb-1 text-primary-500" />
            <div className="text-2xl font-semibold">{summary.caymanianCount}</div>
            <div className="text-xs text-neutral-500">Caymanians Contacted</div>
          </CardContent>
        </Card>
        <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
          <CardContent className="p-4 text-center">
            ...
            <div className="text-xs text-neutral-500">Hired</div>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent className="p-6">
            ...
            <div className="flex flex-wrap gap-3">
              {Object.entries(summary.stageBreakdown).map(([stage, count]) => (
                <div
                  key={stage}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 text-sm"
                >
          ...
          <CardContent className="p-6">
            ...
            <div className="flex flex-wrap gap-3">
              {Object.entries(summary.rejectionBreakdown).map(([reason, count]) => (
                <div
                  key={reason}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-red-500/10 border border-red-300/20 text-sm"
                >
        ...
        <CardContent className="p-6">
          ...
          {intros.length === 0 ? (
            <p className="text-neutral-500 text-sm">
              No candidates have been contacted for this position yet.
            </p>
          ) : (
                ...
                <tbody>
                  {intros.map((intro, i) => (
                    ...
                    >
                      ...
                      <td className="py-2.5 pr-3 text-neutral-600 dark:text-neutral-500">
                      ...
                      <td className="py-2.5 pr-3">
                        {intro.match_score != null
                          ? `${Math.round(Number(intro.match_score))}%`
                          : "--"}
                      </td>
                      <td className="py-2.5 pr-3">
                        <Badge
                      ...
                      <td className="py-2.5 pr-3 text-neutral-300 capitalize">
                        {intro.initiated_by || "employer"}
                      </td>
                      <td className="py-2.5 pr-3 text-neutral-600 dark:text-neutral-500">
                        {intro.rejection_reason
                          ? REJECTION_REASON_LABELS[intro.rejection_reason] ||
                            intro.rejection_reason
                          : "--"}
                      </td>
                  ...
                  <div className="flex-1">
                    ...
                    {a.details && a.details.from && a.details.to && (
                      <div className="text-xs text-neutral-500">
                        {STAGE_LABELS[a.details.from] || a.details.from} &rarr;{" "}
                        {STAGE_LABELS[a.details.to] || a.details.to}
                      </div>
                    )}
