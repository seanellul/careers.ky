    ...
    <div className="mt-3 space-y-3">
      {messages.length > 0 && (
        <div className="space-y-2 max-h-60 overflow-y-auto">
          {messages.map((m) => (
            <div
              key={m.id}
              className={`text-sm p-2 rounded-lg ${m.sender_type === "employer" ? "bg-primary-50 dark:bg-primary-500/15 border border-primary-200 dark:border-primary-500/30 ml-4" : "bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 mr-4"}`}
            >
              <div className="text-xs text-neutral-500 mb-1">
                {m.sender_type === "employer" ? "You" : "Candidate"} &middot;{" "}
      ...
      <div className="flex gap-2">
        <input
          ...
          maxLength={2000}
          className="flex-1 bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 rounded-lg px-3 py-2 text-sm text-neutral-700 dark:text-neutral-300 placeholder:text-neutral-400 focus:outline-none focus:border-primary-300"
        />
...
export default function EmployerDashboardClient({
  ...
  const filteredPostings = useMemo(() => {
    ...
    return jobPostings.filter(
      (p) => p.title.toLowerCase().includes(q) || p.jobId.toLowerCase().includes(q)
    );
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row items-start justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-12 h-12 rounded-xl bg-neutral-100 dark:bg-neutral-800 grid place-items-center">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-neutral-500">Total Intros</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-neutral-500">Accept Rate</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div
                className={`h-10 w-10 rounded-xl ${monthTrend >= 0 ? "bg-emerald-100 dark:bg-emerald-500/20" : "bg-red-100 dark:bg-red-500/20"} grid place-items-center`}
              >
                {monthTrend >= 0 ? (
              ...
              <div>
                ...
                <div className="text-xs text-neutral-500">
                  This Month{" "}
                  {monthTrend !== 0 && (
                    <span className={monthTrend > 0 ? "text-emerald-600" : "text-red-500"}>
                      {monthTrend > 0 ? "+" : ""}
                      {monthTrend}%
                    </span>
                  )}
          ...
          <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700">
            <CardContent className="p-6">
              <h2 className="text-xl font-semibold mb-4 flex items-center gap-2">
                <Briefcase className="w-5 h-5" /> Hiring Pipeline
              </h2>
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-neutral-800 dark:text-neutral-200 truncate">
                          {selectedJobId
                            ? jobPostings.find((p) => p.jobId === selectedJobId)?.title ||
                              selectedJobId
                            : "All Postings"}
                        </div>
                        <div className="text-xs text-neutral-500">
                        ...
                        <div className="p-2 border-b border-neutral-200 dark:border-neutral-700">
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 rounded-lg pl-9 pr-3 py-2 text-sm text-neutral-700 dark:text-neutral-300 placeholder:text-neutral-400 focus:outline-none focus:border-primary-300"
                              autoFocus
                            />
                          </div>
                        </div>
                      ...
                      <div className="max-h-64 overflow-y-auto py-1">
                        ...
                        {filteredPostings.map((p) => (
                          ...
                          >
                            ...
                            <span className="text-xs text-neutral-400 font-mono ml-auto shrink-0">
                              {p.jobId}
                            </span>
                          </button>
                        ))}
                        {filteredPostings.length === 0 && (
                          <div className="px-3 py-4 text-sm text-neutral-500 text-center">
                            No postings match &ldquo;{jobSearch}&rdquo;
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  return (
                    <>
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-sm font-medium transition ${activeStage === "all" ? "bg-primary-50 dark:bg-primary-500/15 text-primary-500 border border-primary-200 dark:border-primary-500/30" : "bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 text-neutral-500 hover:text-neutral-700 dark:text-neutral-300"}`}
                      >
                        All ({jobFiltered.length})
                      </button>
                      {Object.entries(STAGE_LABELS).map(([stage, label]) => {
                        const count = jobFiltered.filter(
                          (i) => (i.stage || "outreach") === stage
                        ).length;
                        return (
                          <button
                            key={stage}
              ...
              {filteredIntros.length > 0 ? (
                <div className="space-y-3">
                  {filteredIntros.slice(0, 15).map((intro) => (
                    <div
                      key={intro.id}
                      className="p-4 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800 space-y-2"
                    >
                      <div>
                        ...
                        <div className="text-xs text-neutral-500">
                          {intro.initiated_by === "candidate" ? "Expressed interest" : "Sent"}{" "}
                          ...
                          {intro.responded_at &&
                            ` · Responded ${new Date(intro.responded_at).toLocaleDateString("en-US", { month: "short", day: "numeric" })}`}
                        </div>
                      </div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <select
                          ...
                          className="bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 rounded-lg px-2 py-1 text-xs text-neutral-700 dark:text-neutral-300"
                        >
                        ...
                        {intro.job_title ? (
                          ...
                          <Badge className="bg-white dark:bg-neutral-800 shadow-sm text-neutral-500 border-neutral-200 dark:border-neutral-700 text-xs">
                            General
                          </Badge>
                        )}
                      </div>
                      ...
                      {intro.initiated_by === "candidate" && intro.status === "pending" && (
                        <div className="flex gap-2 mt-2">
                          <Button
                            ...
                            onClick={() => handleRespondToInterest(intro.id, true)}
                            disabled={respondingInterest === intro.id}
                          >
                          ...
                          <Button
                            ...
                            variant="secondary"
                            className="gap-1.5 flex-1"
                            onClick={() => handleRespondToInterest(intro.id, false)}
                            disabled={respondingInterest === intro.id}
                          >
                      ...
                      {intro.rejection_reason && (
                        <div className="text-xs text-red-500 mt-1 flex items-center gap-1">
                          ...
                          Rejection:{" "}
                          {REJECTION_REASONS.find((r) => r.value === intro.rejection_reason)
                            ?.label || intro.rejection_reason}
                          {intro.rejection_notes && (
                            <span className="text-neutral-500 ml-1">— {intro.rejection_notes}</span>
                          )}
                        </div>
                      )}
                      {intro.status === "accepted" && intro.candidate_email && (
                        <div className="flex items-center gap-2 text-sm text-primary-600 dark:text-primary-400 mt-2 p-2 rounded-lg bg-primary-50 dark:bg-primary-500/10 border border-primary-200 dark:border-primary-500/30">
                          ...
                          <a href={`mailto:${intro.candidate_email}`} className="hover:underline">
                      ...
                      {intro.status === "accepted" && (
                        <div className="mt-2">
                          ...
                          {expandedMessages[intro.id] && (
                            <EmployerMessageThread introId={intro.id} />
                          )}
                        </div>
                      )}
                    </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-neutral-500 mb-3">
                    {activeStage === "all"
                      ...
                      : `No introductions in ${STAGE_LABELS[activeStage]} stage.`}
                  </p>
                  <Link href="/employer/talent">
                    <Button className="gap-2">
                      <Search className="w-4 h-4" /> Search Talent
                      ...
                      <div className="flex-1">
                        ...
                        {a.details && Object.keys(a.details).length > 0 && (
                          <div className="text-xs text-neutral-500">
                            {a.details.from &&
                              a.details.to &&
                              `${STAGE_LABELS[a.details.from] || a.details.from} → ${STAGE_LABELS[a.details.to] || a.details.to}`}
                          </div>
                        )}
                        <div className="text-xs text-neutral-500">
        ...
        <div className="space-y-6">
            ...
            <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700">
              <CardContent className="p-6">
                <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                  <List className="w-5 h-5" /> Shortlists
                </h3>
                <div className="space-y-2">
                  {shortlists.map((sl) => (
                    <Link
                      ...
                      className="flex items-center justify-between p-3 rounded-xl bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 hover:border-primary-200 dark:border-primary-500/30 transition text-sm"
                    >
                      <span>{sl.name}</span>
                ...
                <Link href="/employer/shortlists">
                  <Button variant="secondary" size="sm" className="w-full mt-3 gap-1">
                    View All
                  </Button>
                </Link>
              </CardContent>
          ...
          {savedSearches && savedSearches.length > 0 && (
            <Card className="bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700">
              <CardContent className="p-6">
                <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                  <BookmarkPlus className="w-5 h-5" /> Saved Searches
                </h3>
                <div className="space-y-2">
                  {savedSearches.map((ss) => (
                    <Link
                      key={ss.id}
                      href={`/employer/talent?${new URLSearchParams(ss.filters || {}).toString()}`}
                      className="flex items-center gap-2 p-3 rounded-xl bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 hover:border-primary-200 dark:border-primary-500/30 transition text-sm"
                    >
      ...
      {rejectionModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl">
            <h3 className="text-lg font-semibold mb-1 flex items-center gap-2">
              <AlertTriangle className="w-5 h-5 text-red-500" /> Rejection Reason
            </h3>
            <p className="text-sm text-neutral-500 mb-4">
              WORC compliance requires documenting why a candidate was not selected.
            </p>
            <div className="space-y-3">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 rounded-lg px-3 py-2 text-sm text-neutral-700 dark:text-neutral-300"
                >
              ...
              <div className="flex gap-2 pt-2">
                ...
                <Button
                  ...
                  disabled={!rejectionReason || updatingStage}
