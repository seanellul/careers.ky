...
export default function EmployerProfileClient({ profile, perks: perkGroups, workTypes: wtObj, eduTypes: etObj, expTypes: exObj, locTypes: ltObj }) {
  ...
  const Bar = ({ label, value, total, color = "bg-cyan-400" }) => (
    <div className="flex items-center gap-2">
      <div className="w-16 sm:w-24 md:w-32 text-xs text-neutral-600 truncate" title={label}>{label}</div>
      ...
      <div className="relative">
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="relative -mt-12 sm:-mt-16 md:-mt-20 flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
            {/* Logo */}
            {employer.logo_url ? (
              <img src={employer.logo_url} alt={employer.name} className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-2xl object-contain bg-white border-4 border-[#FEFCF3] shadow-xl" />
            ) : (
              <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-2xl bg-neutral-50 border-4 border-[#FEFCF3] grid place-items-center shadow-xl">
            ...
            <div className="flex-1 pt-2 sm:pt-4">
              ...
              <div className="flex items-center gap-3 mt-3 flex-wrap">
                {stats.activePostings > 0 && (
                  <Badge className="bg-emerald-50 text-emerald-600 border-emerald-200">Currently Hiring</Badge>
                )}
                {employer.claimed && (
                  <Badge className="bg-primary-50 text-primary-500 border-primary-200">Verified</Badge>
                )}
              </div>
              ...
              <div className="flex items-center gap-3 mt-3 flex-wrap">
                {employer.website && (
                  <a href={employer.website} target="_blank" rel="noreferrer" className="text-sm text-neutral-500 hover:text-neutral-900 flex items-center gap-1 transition">
                    <Globe className="w-3.5 h-3.5" /> Website
                  </a>
                )}
                {socialLinks.linkedin && (
                  <a href={socialLinks.linkedin} target="_blank" rel="noreferrer" className="text-neutral-500 hover:text-neutral-900 transition">
                    <Linkedin className="w-4 h-4" />
                  </a>
                )}
                {socialLinks.twitter && (
                  <a href={socialLinks.twitter} target="_blank" rel="noreferrer" className="text-neutral-500 hover:text-neutral-900 transition">
                    <Twitter className="w-4 h-4" />
                  </a>
                )}
                {socialLinks.instagram && (
                  <a href={socialLinks.instagram} target="_blank" rel="noreferrer" className="text-neutral-500 hover:text-neutral-900 transition">
        ...
        {sections.about && (employer.description || employer.company_size || employer.headquarters) && (
          <section>
            ...
            {employer.description && (
              <p className="text-neutral-600 leading-relaxed max-w-3xl whitespace-pre-line">{employer.description}</p>
            )}
                  ...
                  <div key={group.id}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {group.perks.map((perk, i) => (
                        <Badge key={i} className="bg-neutral-50 border-neutral-200 text-neutral-700 px-3 py-1.5">
                          {perk.label}
                        </Badge>
                      ))}
                    </div>
          ...
          <section>
            ...
            {activePostings.length > 0 && (
              <div className="relative mb-4 max-w-md">
                ...
                <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by job title..." className="pl-10 bg-neutral-50 border-neutral-200" />
              </div>
            )}
            ...
            {filteredJobs.length > 0 ? (
              <div className="grid gap-3">
                {filteredJobs.map((p, i) => (
                  <Card key={p.cJobId || i} className="bg-neutral-50 border-neutral-200 hover:border-primary-300 transition">
        ...
        {sections.insights && stats.totalPostings > 0 && (
          <section>
            ...
            >
              ...
              <Badge className="bg-neutral-50 border-neutral-200 text-neutral-500">{stats.totalPostings} postings</Badge>
            ...
            {insightsOpen && (
              <div className="mt-6 space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <Card className="bg-neutral-50 border-neutral-200"><CardContent className="p-4 text-center"><div className="text-2xl font-semibold">{stats.totalPostings}</div><div className="text-sm text-neutral-500">Total Posts</div></CardContent></Card>
                  <Card className="bg-neutral-50 border-neutral-200"><CardContent className="p-4 text-center"><div className="text-2xl font-semibold">{stats.activePostings}</div><div className="text-sm text-neutral-500">Active Now</div></CardContent></Card>
                  {stats.avgSalary > 0 && <Card className="bg-neutral-50 border-neutral-200"><CardContent className="p-4 text-center"><div className="text-xl font-semibold">CI$ {Math.round(stats.avgSalary).toLocaleString()}</div><div className="text-sm text-neutral-500">Avg Salary</div></CardContent></Card>}
                  {stats.maxSalary > 0 && <Card className="bg-neutral-50 border-neutral-200"><CardContent className="p-4 text-center"><div className="text-xl font-semibold">CI$ {Math.round(stats.maxSalary).toLocaleString()}</div><div className="text-sm text-neutral-500">Max Salary</div></CardContent></Card>}
                </div>
                    ...
                    <CardContent className="p-6">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {industries.map(([name, count]) => (
                          <Badge key={name} className="bg-neutral-50 border-neutral-200 text-neutral-600">{name} ({count})</Badge>
                        ))}
                      </div>
                    </CardContent>
                  </Card>
