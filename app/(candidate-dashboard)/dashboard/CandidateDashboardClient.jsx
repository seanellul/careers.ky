...
function ProfilePicture({ url, name }) {
  ...
  return (
    <div className="w-14 h-14 rounded-full bg-gradient-to-br from-cyan-500/30 to-purple-500/30 grid place-items-center text-xl font-semibold text-cyan-300 border-2 border-white/10">
      {initial}
    </div>
  );
}
    ...
    <div>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">
            Welcome back, <span className="text-cyan-300">{candidate.name || "there"}</span>
          </h1>
          {candidate.headline && (
            <p className="text-neutral-400 text-sm mt-1">{candidate.headline}</p>
          )}
        </div>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-neutral-400">Pending Intros</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-neutral-400">Accepted Intros</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-neutral-400">Jobs Interested In</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-3 sm:p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-neutral-400">Profile Strength</div>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <Card className="bg-white/5 border-white/10">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/dashboard/introductions" className="text-sm text-cyan-300 hover:underline flex items-center gap-1">
              ...
              {activePending.length === 0 && activeAccepted.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-neutral-400 text-sm">No active introductions yet.</p>
                ...
                <div className="space-y-3">
                  {[...activePending, ...activeAccepted.slice(0, 3)].map((intro) => (
                    <div key={intro.id} className={`p-4 rounded-xl border transition ${intro.status === "pending" ? "border-yellow-300/20 bg-yellow-500/5" : "border-white/10 bg-white/5"}`}>
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {intro.job_title && <div className="text-xs text-neutral-400">{intro.job_title}</div>}
                          </div>
                        </div>
          ...
          <Card className="bg-white/5 border-white/10">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href="/dashboard/interests" className="text-sm text-cyan-300 hover:underline flex items-center gap-1">
                  View all <ChevronRight className="w-3 h-3" />
                </Link>
              </div>
              ...
              {jobInterests.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-neutral-400 text-sm">You haven&apos;t expressed interest in any jobs yet.</p>
                  <Link href="/careers" className="text-cyan-300 text-sm hover:underline mt-2 inline-block">Browse Jobs</Link>
                </div>
              ) : (
                <div className="space-y-2">
                  {jobInterests.slice(0, 5).map((ji) => (
                    <div key={ji.id} className="flex items-center gap-3 p-3 rounded-lg bg-white/5 border border-white/10">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-neutral-400 truncate">{ji.employer_name}</div>
                      </div>
        ...
        <div className="space-y-6">
          {/* Profile Completion */}
          {score < 80 && (
            <Card className="bg-white/5 border-cyan-300/20">
              <CardContent className="p-6">
                ...
                <p className="text-xs text-neutral-400 mb-3">{score}% complete — {missing.length} items remaining</p>
                <ul className="space-y-1.5 mb-4">
                  {missing.slice(0, 4).map((item) => (
                    <li key={item} className="text-xs text-neutral-300 flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-cyan-300/50" />
                      {item}
                    </li>
                  ))}
                  {missing.length > 4 && (
          ...
          <Card className="bg-white/5 border-white/10">
            <CardContent className="p-6">
              ...
              <div className="space-y-2">
                <Link href="/careers" className="flex items-center gap-3 p-3 rounded-lg bg-white/5 border border-white/10 hover:border-cyan-300/30 transition text-sm">
                  <Search className="w-4 h-4 text-cyan-300" />
                  <span>Browse Jobs</span>
                  <ArrowUpRight className="w-3.5 h-3.5 ml-auto text-neutral-500" />
                </Link>
                <Link href="/profile" className="flex items-center gap-3 p-3 rounded-lg bg-white/5 border border-white/10 hover:border-cyan-300/30 transition text-sm">
                  <User className="w-4 h-4 text-cyan-300" />
                  <span>Edit Profile</span>
                  <ArrowUpRight className="w-3.5 h-3.5 ml-auto text-neutral-500" />
                </Link>
                <Link href="/dashboard/alerts" className="flex items-center gap-3 p-3 rounded-lg bg-white/5 border border-white/10 hover:border-cyan-300/30 transition text-sm">
          ...
          <Card className="bg-white/5 border-white/10">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <Link href="/notifications" className="text-xs text-cyan-300 hover:underline">View all</Link>
              </div>
              {notifications.length === 0 ? (
                ...
                <div className="space-y-2">
                  {notifications.map((n) => (
                    <div key={n.id} className={`p-2 rounded-lg text-xs ${n.read_at ? "bg-white/5" : "bg-cyan-500/5 border border-cyan-300/10"}`}>
                      <div className="font-medium text-neutral-200">{n.title}</div>
                      <div className="text-neutral-400 mt-0.5">{n.body}</div>
