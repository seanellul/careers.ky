  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <Badge
          className={`sm:hidden ${score >= 80 ? "bg-emerald-50 text-emerald-600 border-emerald-300" : score >= 50 ? "bg-yellow-50 text-yellow-700 border-yellow-300" : "bg-red-50 text-red-500 border-red-300"}`}
        >
          {score}%
        </Badge>
      </div>
              ...
              {activePending.length === 0 && activeAccepted.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-neutral-400 text-xs mt-1">
                    When employers want to connect, they&apos;ll appear here.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {[...activePending, ...activeAccepted.slice(0, 3)].map((intro) => (
                    <div
                      key={intro.id}
                      className={`p-4 rounded-xl border-2 transition ${intro.status === "pending" ? "border-yellow-400 bg-yellow-50 dark:bg-yellow-500/10 dark:border-yellow-500/40" : "border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800"}`}
                    >
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {intro.job_title && (
                              <div className="text-xs text-neutral-500">{intro.job_title}</div>
                            )}
                          </div>
                        </div>
              ...
              {jobInterests.length === 0 ? (
                ...
                <div className="space-y-2">
                  {jobInterests.slice(0, 5).map((ji) => (
                    <Link
                      key={ji.id}
                      href={`/jobs/${ji.job_id || ji.id}`}
                      className="flex items-center gap-3 p-3 rounded-lg bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 hover:border-primary-300 dark:hover:border-primary-500/30 hover:shadow-sm transition"
                    >
            ...
            <Card className="border-primary-300 dark:border-primary-500/40 shadow-md">
              <CardContent className="p-6">
                ...
                <p className="text-xs text-neutral-500 mb-3">
                  {score}% complete — {missing.length} items remaining
                </p>
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <div className="space-y-2">
                <Link
                  href="/careers"
                  className="flex items-center gap-3 p-3 rounded-lg border border-neutral-200 dark:border-neutral-700 hover:border-primary-300 dark:hover:border-primary-500/30 hover:shadow-sm transition text-sm bg-neutral-50 dark:bg-neutral-800"
                >
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-3">
                ...
                <Link href="/notifications" className="text-xs text-primary-500 hover:underline">
                  View all
                </Link>
              </div>
              {notifications.length === 0 ? (
                ...
                <div className="space-y-2">
                  {notifications.map((n) => (
                    <div
                      key={n.id}
                      className={`p-3 rounded-lg text-xs border ${n.read_at ? "bg-neutral-50 dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700" : "bg-primary-50 dark:bg-primary-500/10 border-primary-300 dark:border-primary-500/30"}`}
                    >
