...
export default function JobDetailClient({
  ...
  const Bar = ({ label, value, total, color = "bg-primary-300" }) => (
    <div className="flex items-center gap-2">
      <div className="w-20 sm:w-32 text-xs text-neutral-600 dark:text-neutral-400 truncate" title={label}>{label}</div>
  ...
  return (
    <div className={`${t.page} w-full`}>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Hero */}
        <div className="mb-8">
          <Badge className="mb-3 bg-primary-50 dark:bg-primary-500/15 text-primary-500 border-primary-200 dark:border-primary-500/30">CISCO Unit {unit.id}</Badge>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 text-neutral-500 text-xs font-medium hover:border-neutral-300 dark:hover:border-neutral-600 hover:text-neutral-900 dark:hover:text-neutral-100 transition min-h-[36px]"
            >
          ...
          {stats && (
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4 mt-6">
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"><CardContent className="p-4 text-center"><Briefcase className="w-5 h-5 text-primary-500 mx-auto mb-2" /><div className="text-2xl font-semibold">{stats.count}</div><div className="text-sm text-neutral-500">Job Posts</div></CardContent></Card>
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"><CardContent className="p-4 text-center"><DollarSign className="w-5 h-5 text-primary-500 mx-auto mb-2" /><div className="text-2xl font-semibold">CI$ {Math.round(stats.mean).toLocaleString()}</div><div className="text-sm text-neutral-500">Average Salary</div></CardContent></Card>
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"><CardContent className="p-4 text-center"><TrendingUp className="w-5 h-5 text-primary-500 mx-auto mb-2" /><div className="text-2xl font-semibold">CI$ {Math.round(stats.max).toLocaleString()}</div><div className="text-sm text-neutral-500">Max Salary</div></CardContent></Card>
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"><CardContent className="p-4 text-center"><MapPin className="w-5 h-5 text-primary-500 mx-auto mb-2" /><div className="text-2xl font-semibold">Cayman</div><div className="text-sm text-neutral-500">Location</div></CardContent></Card>
            </div>
          )}
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            {/* Description */}
            <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"><CardContent className="p-6"><h2 className="text-2xl font-semibold mb-4">Job Description</h2><p className="text-neutral-600 dark:text-neutral-400 leading-relaxed">{unit.description}</p></CardContent></Card>

            {/* Tasks */}
            {unit.tasks && (
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"><CardContent className="p-6"><h2 className="text-2xl font-semibold mb-4">Key Tasks & Responsibilities</h2><div className="text-neutral-600 dark:text-neutral-400 whitespace-pre-line leading-relaxed">{unit.tasks}</div></CardContent></Card>
            )}
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="space-y-2">{Object.entries(stats.dist.work).map(([k, v]) => <Bar key={k} label={workTypes.get(k) || k} value={v} total={stats.count} color="bg-primary-300" />)}</div>
                    </div>
                    <div>
                      <h3 className="text-lg font-medium mb-3 flex items-center gap-2"><BookOpen className="w-5 h-5" /> Education Requirements</h3>
                      <div className="space-y-2">{Object.entries(stats.dist.edu).map(([k, v]) => <Bar key={k} label={eduTypes.get(k) || k} value={v} total={stats.count} color="bg-primary-200" />)}</div>
                    </div>
                    <div>
                      <h3 className="text-lg font-medium mb-3 flex items-center gap-2"><Clock className="w-5 h-5" /> Experience Levels</h3>
                      <div className="space-y-2">{Object.entries(stats.dist.exp).map(([k, v]) => <Bar key={k} label={expTypes.get(k) || k} value={v} total={stats.count} color="bg-primary-300" />)}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                <CardContent className="p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {skills.map((s) => (
                      <Badge key={s.id} className="bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 border-neutral-200 dark:border-neutral-700">
                        {s.name}
                      </Badge>
                    ))}
                  </div>
        ...
        <div className="mt-12 pt-8 border-t border-neutral-200 dark:border-neutral-700">
          ...
          <div className="space-y-4 mb-6">
            <div className="relative">
              ...
              <Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by title or employer..." className="pl-10 bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700" />
            </div>
            {showFilters && (
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                <CardContent className="p-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="w-full bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 rounded-lg px-3 py-2 text-sm">
                    ...
                    <div>
                      ...
                      <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="w-full bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 rounded-lg px-3 py-2 text-sm">
          ...
          <div className="grid gap-4">
            {filteredPostings.map((p, i) => (
              <Card key={p.cJobId || i} className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700 hover:border-primary-300 transition group">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="text-xl font-semibold group-hover:text-primary-500 transition">{p.cTitle}</h3>
                        {p.isActive ? <Badge className="bg-primary-50 dark:bg-primary-500/15 text-primary-700 dark:text-primary-300 border-primary-200 dark:border-primary-500/30">Active</Badge> : <Badge className="bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 border-neutral-200 dark:border-neutral-700">Closed</Badge>}
                      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    ...
                    <div><div className="text-xs text-neutral-500 mb-1">Requirements</div><div className="flex gap-1"><Badge className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700 text-xs">{eduTypes.get(p.sEducation) || p.sEducation}</Badge></div></div>
                  </div>
          ...
          {filteredPostings.length === 0 && (
            <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"><CardContent className="p-12 text-center"><Search className="w-12 h-12 mx-auto mb-4 opacity-50" /><h3 className="text-lg font-medium mb-2">No job postings found</h3><p className="text-neutral-500">Try adjusting your search criteria.</p></CardContent></Card>
          )}
        </div>
      ...
      {showCareerPlan && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 backdrop-blur-sm overflow-y-auto py-4 px-2 sm:py-8 sm:px-4">
          <div className="relative w-full max-w-4xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded-2xl shadow-2xl">
            <div className="sticky top-0 z-20 bg-white/95 backdrop-blur border-b border-neutral-200 dark:border-neutral-700 p-6 rounded-t-2xl">
              <div className="flex items-start justify-between">
                ...
                <button onClick={() => setShowCareerPlan(false)} className="p-2 rounded-lg bg-white dark:bg-neutral-800 shadow-sm hover:bg-neutral-100 dark:hover:bg-neutral-800 transition"><X className="w-6 h-6" /></button>
              </div>
            </div>
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"><CardContent className="p-4 text-center"><div className="text-2xl font-semibold text-primary-500">{employerData.length}</div><div className="text-sm text-neutral-500">Employers</div></CardContent></Card>
                <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"><CardContent className="p-4 text-center"><div className="text-2xl font-semibold text-primary-500">{employerData.filter((e) => e.isActiveHiring).length}</div><div className="text-sm text-neutral-500">Active Hirers</div></CardContent></Card>
                <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"><CardContent className="p-4 text-center"><div className="text-2xl font-semibold text-primary-500">{stats ? stats.count : 0}</div><div className="text-sm text-neutral-500">Total Postings</div></CardContent></Card>
                <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"><CardContent className="p-4 text-center"><div className="text-2xl font-semibold text-primary-500">CI$ {stats ? Math.round(stats.mean).toLocaleString() : 0}</div><div className="text-sm text-neutral-500">Avg Salary</div></CardContent></Card>
              </div>
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                    {employerData.map((employer, idx) => (
                      <div key={idx}
                        className={`p-4 rounded-lg border transition ${employer.isActiveHiring ? "bg-primary-50 dark:bg-primary-500/15 border-primary-200 dark:border-primary-500/30 hover:bg-primary-100 cursor-pointer" : "bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700"}`}
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              {employer.isActiveHiring && <Badge className="bg-primary-50 dark:bg-primary-500/15 text-primary-700 dark:text-primary-300 border-primary-200 dark:border-primary-500/30 shrink-0"><AlertCircle className="w-3 h-3 mr-1" /> Actively Hiring</Badge>}
                            </div>
                            <div className="flex flex-wrap items-center gap-3 text-sm text-neutral-500">
                              <span>{employer.totalPostings} posting{employer.totalPostings !== 1 ? "s" : ""}</span>
                              {employer.activePostings > 0 && <span className="text-primary-500">{employer.activePostings} active</span>}
                            </div>
                            {employer.isActiveHiring && <div className="mt-2 text-xs text-primary-500/80">Click to view active job postings</div>}
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              <Card className="bg-gradient-to-r from-primary-50 to-primary-100 border-primary-200 dark:border-primary-500/30">
                <CardContent className="p-6">
