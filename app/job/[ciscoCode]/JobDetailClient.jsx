...
export default function JobDetailClient({
  ...
  const filteredPostings = useMemo(() => {
    let filtered = postings.filter((p) => {
      if (searchQuery) {
        ...
        if (!p.cTitle?.toLowerCase().includes(q) && !p.Employer?.toLowerCase().includes(q))
          return false;
      }
  ...
  const formatSalary = (p) => {
    ...
    if (p.fMinSalary && p.fMaxSalary)
      return `${p.Currency || "CI$"} ${p.fMinSalary.toLocaleString()} - ${p.fMaxSalary.toLocaleString()}`;
  ...
  const Bar = ({ label, value, total, color = "bg-primary-300" }) => (
    <div className="flex items-center gap-2">
      <div
        className="w-20 sm:w-32 text-xs text-neutral-600 dark:text-neutral-400 truncate"
        title={label}
      >
        {label}
      </div>
      ...
      <div className="w-12 sm:w-16 text-xs text-neutral-500 text-right">
  ...
  return (
    <div className={`${t.page} w-full`}>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          <Link href="/career-tracks">
            <Button variant="secondary" className="gap-2">
              <ChevronLeft className="w-4 h-4" /> Back to Career Tracks
            </Button>
          </Link>
        </div>
        ...
        {/* Hero */}
        <div className="mb-8">
          <Badge className="mb-3 bg-primary-50 dark:bg-primary-500/15 text-primary-500 border-primary-200 dark:border-primary-500/30">
            CISCO Unit {unit.id}
          </Badge>
          <h1 className="text-4xl font-semibold tracking-tight">
            {unit.title} <span className="text-primary-500">Jobs in Cayman</span>
          </h1>
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4 mt-6">
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                <CardContent className="p-4 text-center">
                  ...
                  <div className="text-sm text-neutral-500">Job Posts</div>
                </CardContent>
              </Card>
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                <CardContent className="p-4 text-center">
                  ...
                  <div className="text-2xl font-semibold">
                    CI$ {Math.round(stats.mean).toLocaleString()}
                  </div>
              ...
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                <CardContent className="p-4 text-center">
                  ...
                  <div className="text-2xl font-semibold">
                    CI$ {Math.round(stats.max).toLocaleString()}
                  </div>
                  <div className="text-sm text-neutral-500">Max Salary</div>
                </CardContent>
              </Card>
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                <CardContent className="p-4 text-center">
                  ...
                  <div className="space-y-6">
                    <div>
                      <h3 className="text-lg font-medium mb-3 flex items-center gap-2">
                        <Briefcase className="w-5 h-5" /> Work Type Distribution
                      </h3>
                      <div className="space-y-2">
                        {Object.entries(stats.dist.work).map(([k, v]) => (
                          <Bar
                            key={k}
                            label={workTypes.get(k) || k}
                            value={v}
                            total={stats.count}
                    ...
                    <div>
                      <h3 className="text-lg font-medium mb-3 flex items-center gap-2">
                        <BookOpen className="w-5 h-5" /> Education Requirements
                      </h3>
                      <div className="space-y-2">
                        {Object.entries(stats.dist.edu).map(([k, v]) => (
                          <Bar
                            key={k}
                            label={eduTypes.get(k) || k}
                            value={v}
                            total={stats.count}
                    ...
                    <div>
                      <h3 className="text-lg font-medium mb-3 flex items-center gap-2">
                        <Clock className="w-5 h-5" /> Experience Levels
                      </h3>
                      <div className="space-y-2">
                        {Object.entries(stats.dist.exp).map(([k, v]) => (
                          <Bar
                            key={k}
                            label={expTypes.get(k) || k}
                            value={v}
                            total={stats.count}
                            color="bg-primary-300"
                          />
          ...
          <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        CI$ {Math.round(stats.min).toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      <span className="text-neutral-500">Maximum</span>
                      <span className="font-medium">
                        CI$ {Math.round(stats.max).toLocaleString()}
                      </span>
                    </div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3">
                    {relatedJobs.map((job, i) => (
                      <div
                        key={i}
                        className="p-3 bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 rounded-lg"
                      >
                        <div className="font-medium text-sm">{job.cTitle}</div>
                        <div className="text-xs text-neutral-500">
                          Occupation #{job.sOccupation}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
              <CardContent className="p-6">
                ...
                <div className="space-y-3">
                  <Button className="w-full gap-2" onClick={() => setShowCareerPlan(true)}>
                    <FileText className="w-4 h-4" /> Build Career Plan
                  </Button>
                  <Link href={`/jobs?q=${encodeURIComponent(unit.title)}`} className="block">
                    <Button variant="secondary" className="w-full gap-2">
                      <ExternalLink className="w-4 h-4" /> Search Live Jobs
                    </Button>
                  </Link>
                </div>
              </CardContent>
        ...
        <div className="mt-12 pt-8 border-t border-neutral-200 dark:border-neutral-700">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-neutral-500">
                {filteredPostings.length} of {postings.length} total postings
              </p>
            </div>
            <Button
              variant="secondary"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 rounded-lg px-3 py-2 text-sm"
                      >
          ...
          <div className="grid gap-4">
            {filteredPostings.map((p, i) => (
              <Card
                key={p.cJobId || i}
                className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700 hover:border-primary-300 transition group"
              >
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="text-xl font-semibold group-hover:text-primary-500 transition">
                          {p.cTitle}
                        </h3>
                        {p.isActive ? (
                          <Badge className="bg-primary-50 dark:bg-primary-500/15 text-primary-700 dark:text-primary-300 border-primary-200 dark:border-primary-500/30">
                            Active
                          </Badge>
                        ) : (
                          <Badge className="bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 border-neutral-200 dark:border-neutral-700">
                            Closed
                          </Badge>
                        )}
                      </div>
                      <div className="flex items-center gap-4 text-sm text-neutral-500 mb-3">
                        ...
                        >
                          <Building2 className="w-4 h-4" /> {p.Employer}
                        </Link>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    <div>
                      ...
                      <div className="text-sm font-medium">{formatDate(p.createdDate)}</div>
                    </div>
                    <div>
                      <div className="text-xs text-neutral-500 mb-1">Start Date</div>
                      <div className="text-sm font-medium">{formatDate(p.startDate)}</div>
                    </div>
                    <div>
                      <div className="text-xs text-neutral-500 mb-1">End Date</div>
                      <div className="text-sm font-medium">{formatDate(p.endDate)}</div>
                    </div>
                    <div>
                      <div className="text-xs text-neutral-500 mb-1">Requirements</div>
                      <div className="flex gap-1">
                        <Badge className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700 text-xs">
                          {eduTypes.get(p.sEducation) || p.sEducation}
                        </Badge>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center justify-between pt-4 border-t border-neutral-200 dark:border-neutral-700">
                    <div className="text-sm text-neutral-500">
                      <Calendar className="w-4 h-4 inline mr-1" /> Job ID: {p.cJobId}
                    </div>
                    {p.isActive && (
        ...
        <div className="mt-12 pt-8 border-t border-neutral-200 dark:border-neutral-700">
          ...
          <p className="text-neutral-600 dark:text-neutral-400 leading-relaxed mb-4">
            {unit.title} positions in the Cayman Islands offer unique opportunities in a thriving
            offshore financial center. With a growing economy and international business
            environment, Cayman provides excellent career prospects for professionals in this field.
          </p>
          <p className="text-neutral-600 dark:text-neutral-400 leading-relaxed mb-4">
            The Cayman Islands job market for {unit.title} roles is characterized by competitive
            salaries, tax-free income, and opportunities to work with leading international
            companies.
          </p>
          <h3 className="text-xl font-semibold mb-3">
            Why Choose Cayman for {unit.title} Careers?
          </h3>
              ...
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-3xl font-semibold mb-2 flex items-center gap-3">
                    <FileText className="w-8 h-8 text-primary-500" /> Career Plan: {unit.title}
                  </h2>
                  <p className="text-neutral-500">
                    Your roadmap to entering this career track in Cayman
                  </p>
                </div>
                <button
                  onClick={() => setShowCareerPlan(false)}
                  className="p-2 rounded-lg bg-white dark:bg-neutral-800 shadow-sm hover:bg-neutral-100 dark:hover:bg-neutral-800 transition"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                  <CardContent className="p-4 text-center">
                    ...
                    <div className="text-sm text-neutral-500">Employers</div>
                  </CardContent>
                </Card>
                <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                  <CardContent className="p-4 text-center">
                    ...
                    <div className="text-sm text-neutral-500">Active Hirers</div>
                  </CardContent>
                </Card>
                <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                  <CardContent className="p-4 text-center">
                    ...
                    <div className="text-sm text-neutral-500">Total Postings</div>
                  </CardContent>
                </Card>
                <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                  <CardContent className="p-4 text-center">
              ...
              <Card className="bg-white dark:bg-neutral-800 shadow-sm border-neutral-200 dark:border-neutral-700">
                <CardContent className="p-6">
                  <h3 className="text-xl font-semibold mb-4 flex items-center gap-2">
                    <Building2 className="w-5 h-5 text-primary-500" /> Employers in Cayman Islands
                  </h3>
                        ...
                        onClick={
                          employer.isActiveHiring
                            ? () => {
                                ...
                                router.push(`/jobs?employer=${encodeURIComponent(employer.name)}`);
                              }
                            : undefined
                        }
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              {employer.isActiveHiring && (
                                <Badge className="bg-primary-50 dark:bg-primary-500/15 text-primary-700 dark:text-primary-300 border-primary-200 dark:border-primary-500/30 shrink-0">
                                  <AlertCircle className="w-3 h-3 mr-1" /> Actively Hiring
                                </Badge>
                              )}
                            </div>
                            <div className="flex flex-wrap items-center gap-3 text-sm text-neutral-500">
                              <span>
                                {employer.totalPostings} posting
                                {employer.totalPostings !== 1 ? "s" : ""}
                              </span>
                            ...
                            {employer.isActiveHiring && (
                              <div className="mt-2 text-xs text-primary-500/80">
                                Click to view active job postings
                              </div>
                            )}
                          </div>
                        </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      onClick={() => {
                        setShowCareerPlan(false);
                        router.push(`/jobs?q=${encodeURIComponent(unit.title)}`);
                      }}
