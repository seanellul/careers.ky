...
export default function AdminOutreachClient({ initialEmployers }) {
  ...
  const statusColors = {
    not_contacted: "border-neutral-700 text-neutral-400",
    contacted: "border-yellow-500/30 text-yellow-400",
    responded: "border-blue-500/30 text-blue-400",
    onboarded: "border-green-500/30 text-green-400",
  };
    ...
    <div>
      ...
      <div className="space-y-3">
        {employers.map((emp) => {
          ...
          return (
            <div key={emp.id} className="bg-neutral-900 border border-white/10 rounded-xl p-4">
              ...
              <div className="flex items-center gap-3 mb-3">
                <select
                  ...
                  className={`text-xs px-2 py-1.5 rounded-lg bg-transparent border ${statusColors[status]} focus:outline-none focus:border-cyan-500/50`}
                >
                  {STATUSES.map((s) => (
              ...
              <div className="flex items-start gap-2">
                <textarea
                  ...
                  rows={1}
                  className="flex-1 text-sm px-3 py-2 bg-neutral-950 border border-white/5 rounded-lg text-neutral-300 placeholder-neutral-600 focus:outline-none focus:border-cyan-500/30 resize-none"
                />
                {editingNotes[emp.slug] !== undefined && editingNotes[emp.slug] !== emp.notes && (
                  <button
                    onClick={() => saveNotes(emp.slug)}
                    disabled={saving === emp.slug}
                    className="p-2 rounded-lg bg-cyan-500/15 text-cyan-300 hover:bg-cyan-500/25 transition"
                    title="Save notes"
                  >
