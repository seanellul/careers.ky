...
export default function AdminOutreachClient({ initialEmployers }) {
  ...
  async function updateStatus(slug, status) {
    ...
    if (res.ok) {
      setEmployers((prev) =>
        prev.map((e) => (e.slug === slug ? { ...e, outreach_status: status } : e))
      );
    }
    setSaving(null);
  }
  ...
  async function saveNotes(slug) {
    ...
    if (res.ok) {
      setEmployers((prev) =>
        prev.map((e) => (e.slug === slug ? { ...e, notes: editingNotes[slug] } : e))
      );
    }
    setSaving(null);
  }
  ...
  return (
    <div>
      ...
      <p className="text-sm text-neutral-500 mb-6">
        Employers not yet on platform, sorted by priority score (interests x3 + active jobs)
      </p>
            ...
            >
              ...
              <div className="flex items-start gap-2">
                <textarea
                  value={notes}
                  onChange={(e) =>
                    setEditingNotes((prev) => ({ ...prev, [emp.slug]: e.target.value }))
                  }
