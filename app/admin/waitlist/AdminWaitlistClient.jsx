...
import { Building2, Calendar, Mail, Users, Search, CheckCircle2, Clock, XCircle } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <Card key={label} className="bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-900/50">
                <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide text-neutral-500">Company</th>
                <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide text-neutral-500">Contact</th>
                <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide text-neutral-500">Size</th>
                <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide text-neutral-500">Hiring For</th>
                <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide text-neutral-500">Joined</th>
                <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wide text-neutral-500">Status</th>
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center text-neutral-500">No entries found</td>
                </tr>
              )}
              {filtered.map(entry => {
                const StatusIcon = STATUS_ICONS[entry.status] || Clock;
                return (
                  <tr key={entry.id} className="border-b border-neutral-100 dark:border-neutral-700/50 hover:bg-neutral-50 dark:hover:bg-neutral-700/30 transition">
                    ...
                    <td className="px-4 py-3">
                      <div className="text-neutral-700 dark:text-neutral-300">{entry.name || "—"}</div>
                    ...
                    <td className="px-4 py-3">
                      <Badge className={`${STATUS_COLORS[entry.status] || STATUS_COLORS.pending} text-xs flex items-center gap-1 w-fit`}>
