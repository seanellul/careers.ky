      ...
      {messages.length > 0 && (
        <div className="space-y-2 max-h-40 sm:max-h-60 overflow-y-auto">
          {messages.map((m) => (
            <div
              key={m.id}
              className={`text-sm p-2 rounded-lg ${m.sender_type === "candidate" ? "bg-primary-50 dark:bg-primary-500/15 border border-primary-200 dark:border-primary-500/30 ml-2 sm:ml-4" : "bg-white dark:bg-neutral-800 shadow-sm border border-neutral-200 dark:border-neutral-700 mr-2 sm:mr-4"}`}
            >
              <div className="text-xs text-neutral-500 mb-1">
                {m.sender_type === "candidate" ? "You" : "Employer"} &middot;{" "}
...
function JobContext({ intro }) {
  ...
  const salary =
    intro.job_min_salary && intro.job_max_salary
  ...
  return (
    <Link
      ...
      className="flex items-center gap-2 text-sm p-3 rounded-lg bg-purple-50 border border-purple-200 hover:border-purple-300 transition mb-3"
    >
      <Briefcase className="w-4 h-4 text-purple-600 shrink-0" />
      <span className="text-purple-700 font-medium truncate">
        {intro.job_title || intro.job_id}
      </span>
      {salary && (
        <>
  ...
  if (initiatedBy === "candidate") {
    return (
      <Badge className="bg-purple-50 text-purple-600 border-purple-200 text-xs">
        <HeartHandshake className="w-3 h-3 mr-1" /> You expressed interest
      </Badge>
    );
  }
  return (
    <Badge className="bg-primary-50 dark:bg-primary-500/15 text-primary-500 border-primary-200 dark:border-primary-500/30 text-xs">
      <Building2 className="w-3 h-3 mr-1" /> Employer reached out
    </Badge>
  );
}
        ...
        <div className="mb-8">
          ...
          <div className="space-y-4">
            {pending.map((intro) => (
              <Card
                key={intro.id}
                className="bg-white dark:bg-neutral-800 shadow-sm border-yellow-200"
              >
                  ...
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {intro.employer_slug && (
                          <Link
                            href={`/employer/${intro.employer_slug}`}
                  ...
                  <CardContent className="p-4">
                    ...
                    {intro.status === "accepted" && intro.employer_contact_email && (
                      <div className="flex items-center gap-2 text-sm text-emerald-600 mt-2 p-2 rounded-lg bg-emerald-50 border border-emerald-200">
                        ...
                        <a
                          href={`mailto:${intro.employer_contact_email}`}
