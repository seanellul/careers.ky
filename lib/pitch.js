...
export const SEGMENTS = {
  Legal: {
    ...
    headline: (c) =>
      `${c.jobCount} open positions. CI$${Math.round(c.hiringVolume * 120000 * 0.2).toLocaleString()} in recruiter fees. There's a better way.`,
  ...
  Finance: {
    ...
    headline: (c) =>
      `${c.jobCount} open roles. CI$${Math.round(c.hiringVolume * 110000 * 0.2).toLocaleString()} in recruiter fees. One platform replaces them all.`,
    ctaPrimary: "Start Free Trial",
    ctaSecondary: "See Your Savings",
    avgSalary: 110000,
    tier: "Enterprise",
  ...
  Hospitality: {
    ...
    headline: (c) =>
      `${c.jobCount} jobs posted. ${c.hiringVolume} hires needed. At CI$7-11K each in recruiter fees, that adds up fast.`,
  ...
  Construction: {
    ...
    headline: (c) =>
      `${c.hiringVolume} hires needed this year. Skip the recruiter. Find Caymanian talent directly.`,
    ctaPrimary: "Start Free Trial",
    ctaSecondary: "Schedule a 15-min Demo",
    avgSalary: 55000,
    tier: "Pro",
  ...
  Retail: {
    ...
    headline: (c) =>
      `${c.jobCount} positions to fill. At 15-25% per hire, you're overpaying. We fixed that.`,
    ctaPrimary: "Start Free Trial",
    ctaSecondary: "See Your Savings",
    avgSalary: 40000,
    tier: "Pro",
  ...
  Other: {
    ...
    headline: (c) =>
      `You posted ${c.jobCount} jobs. Your recruiter charged 15-25% on each hire. We charge $299/month.`,
    ctaPrimary: "Start Free Trial",
    ctaSecondary: "Schedule a 15-min Demo",
    avgSalary: 65000,
    tier: "Pro",
...
export function getSegment(industry) {
  ...
  const key = Object.keys(SEGMENTS).find((k) => industry.toLowerCase().includes(k.toLowerCase()));
...
export function personalizePitchHTML(template, employer) {
  ...
  const headlineHtml = escapeHtml(seg.headline(company));

  const replacements = {
    "{{COMPANY_NAME}}": escapeHtml(employer.name),
    "{{COMPANY_SLUG}}": escapeHtml(employer.slug),
    "{{INDUSTRY}}": escapeHtml(employer.industry || "Your Industry"),
    ...
    '<span class="accent">{{SEGMENT_HEADLINE}}</span>': `<span class="accent">${headlineHtml}</span>`,
  };
