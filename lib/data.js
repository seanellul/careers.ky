...
export async function loadCISCO() {
  const sql = getDb();
  const rows =
    await sql`SELECT cisco_code as "sCISCO", title as "cTitle", description as "cDescription", tasks as "cTasks" FROM cisco_units ORDER BY cisco_code`;
  return rows;
}
...
export async function loadOccupationCisco() {
  const sql = getDb();
  const rows =
    await sql`SELECT occupation_code as "sOccupation", cisco_code as "sCISCO" FROM occupation_cisco`;
  return rows;
}
...
export async function loadJobTitles() {
  const sql = getDb();
  const rows =
    await sql`SELECT occupation_code as "sOccupation", title as "cTitle", title as "cJobTitle" FROM job_titles`;
  return rows;
}
...
export async function loadAggregates() {
  ...
  // Build distribution maps. Marginals (dist) drive the per-card bars; the
  // joint rows (joint) are what combined filters intersect on — collapsing to
  // marginals alone made "Bachelor's AND part-time" unanswerable.
  const distMap = new Map();
  const jointMap = new Map();
  for (const r of detailRows) {
    const code = r.cisco_code;
    if (!distMap.has(code)) distMap.set(code, { work: {}, edu: {}, exp: {} });
    if (!jointMap.has(code)) jointMap.set(code, []);
    ...
    jointMap.get(code).push({ w: wk, e: ed, x: ex, n: Number(r.cnt) });
  }
...
export async function getCiscoUnit(code) {
  const sql = getDb();
  const rows =
    await sql`SELECT cisco_code as "sCISCO", title as "cTitle", description as "cDescription", tasks as "cTasks" FROM cisco_units WHERE cisco_code = ${String(code)}`;
  if (!rows.length) return null;
  const row = rows[0];
  return {
    id: String(row.sCISCO),
    title: row.cTitle,
...
export async function getEmployerProfile(slug) {
  ...
  const postings = await sql`
    SELECT
      ...
      jp.number_of_positions as "numPositions",
      jp.applicant_count as "applicantCount",
      jp.frequency_of_payment as "payFrequency",
      (jp.status = 'Active' AND jp.end_date > NOW()) as "isActive"
    FROM job_postings jp
  ...
  const stats = {
    ...
    avgSalary:
      postings.filter((p) => p.fMeanSalary > 0).reduce((a, p) => a + p.fMeanSalary, 0) /
        Math.max(1, postings.filter((p) => p.fMeanSalary > 0).length) || 0,
  };
...
export async function upsertCandidate(email, data) {
  const sql = getDb();
  // status is the source of truth for ranking; is_caymanian stays in sync
  // for queries that haven't migrated yet. Legacy callers passing only
  // isCaymanian=true still resolve to status='caymanian'.
  const status = data.status || (data.isCaymanian ? "caymanian" : null);
  const isCaymanian = status ? status === "caymanian" : data.isCaymanian || false;
  // profile_type is the source of truth for visibility; is_discoverable
  // stays in sync (anything but 'closed' is discoverable).
  const profileType =
    data.profileType ||
    (data.isDiscoverable != null ? (data.isDiscoverable ? "open" : "closed") : null);
  const isDiscoverable = profileType ? profileType !== "closed" : data.isDiscoverable || false;
  const rows = await sql`
    INSERT INTO candidates (email, name, status, is_caymanian, education_code, experience_code, location_code,
      availability, profile_type, is_discoverable, bio, salary_min, salary_target, salary_negotiable, notice_period,
      work_type_preferences, linkedin_url, resume_summary,
      profile_picture_url, headline, phone, portfolio_url, years_of_experience, preferred_industries, willing_to_relocate)
    VALUES (${email.toLowerCase()}, ${data.name || null}, ${status}, ${isCaymanian},
            ${data.educationCode || null}, ${data.experienceCode || null}, ${data.locationCode || null},
            ${data.availability || "actively_looking"}, ${profileType || "closed"}, ${isDiscoverable}, ${data.bio || null},
            ${data.salaryMin || null}, ${data.salaryTarget || null}, ${data.salaryNegotiable || false}, ${data.noticePeriod || null},
    ...
    ON CONFLICT (email) DO UPDATE SET
      ...
      is_caymanian = CASE
        WHEN EXCLUDED.status IS NOT NULL THEN EXCLUDED.status = 'caymanian'
        ELSE EXCLUDED.is_caymanian
      END,
      ...
      profile_type = CASE
        WHEN ${profileType}::text IS NULL THEN candidates.profile_type
        ELSE EXCLUDED.profile_type
      END,
      is_discoverable = CASE
        WHEN ${profileType}::text IS NULL THEN EXCLUDED.is_discoverable
        ELSE EXCLUDED.profile_type != 'closed'
      END,
...
// --- Blocked employers (hard privacy block, enforced in talent searches) ---

export async function getBlockedEmployers(candidateId) {
  const sql = getDb();
  return sql`
    SELECT be.employer_id, e.name, e.slug
    FROM candidate_blocked_employers be
    JOIN employers e ON e.id = be.employer_id
    WHERE be.candidate_id = ${candidateId}
    ORDER BY e.name
  `;
}

export async function updateBlockedEmployers(candidateId, employerIds) {
  const sql = getDb();
  await sql`DELETE FROM candidate_blocked_employers WHERE candidate_id = ${candidateId}`;
  for (const employerId of employerIds) {
    await sql`
      INSERT INTO candidate_blocked_employers (candidate_id, employer_id)
      VALUES (${candidateId}, ${employerId})
...
export async function searchTalent(filters = {}) {
  ...
  // Enrich with interests and skills (batched — one query each for the whole result set)
...
export async function searchSkills(query, limit = 20) {
  ...
  // Match skills where ANY query word appears in the name
  // e.g. "software developer" matches "Software Development" via the word "software"
...
export async function getJobPostingById(jobId) {
  ...
  const rows = await sql`
    SELECT
      ...
      posted_by_account_id as "postedByAccountId",
      (status = 'Active' AND end_date > NOW()) as "isActive"
    FROM job_postings
    WHERE job_id = ${jobId}
  `;
...
export async function getActiveJobPostingsPaginated(page = 1, pageSize = 12, filters = {}) {
  ...
  const selectCols = sql`
    ...
    industry as "sIndustry", source as "source"
  `;

  if (sort === "salary_desc") {
    rows =
      await sql`SELECT ${selectCols} FROM job_postings ${whereClause} ORDER BY mean_salary DESC NULLS LAST LIMIT ${pageSize} OFFSET ${offset}`;
  } else if (sort === "salary_asc") {
    rows =
      await sql`SELECT ${selectCols} FROM job_postings ${whereClause} ORDER BY mean_salary ASC NULLS LAST LIMIT ${pageSize} OFFSET ${offset}`;
  } else {
    rows =
      await sql`SELECT ${selectCols} FROM job_postings ${whereClause} ORDER BY created_date DESC LIMIT ${pageSize} OFFSET ${offset}`;
  }
...
export async function checkEmployerForJob(jobId) {
  ...
  `;
  return rows[0]
    ? { employerAccountId: rows[0].employer_account_id, employerName: rows[0].employer_name }
    : null;
}
...
export async function getAdminInterests({ limit = 300 } = {}) {
  const sql = getDb();
  return sql`
    SELECT ji.*,
...
export async function getAdminOutreachList({ limit = 300 } = {}) {
  const sql = getDb();
  return sql`
    SELECT
      e.id, e.name, e.slug, e.claimed,
...
export async function getAdminCandidates({ limit = 500 } = {}) {
  const sql = getDb();
...
export async function getAdminEmployers() {
  ...
  const [statsRows, employers] = await Promise.all([
    ...
    sql`
      SELECT
        e.id, e.name, e.slug, e.claimed, e.tier, e.created_at,
        ...
        COALESCE(
          json_agg(DISTINCT jsonb_build_object(
            'id', ea.id, 'email', ea.email, 'name', ea.name,
            'role', ea.role, 'verification_status', ea.verification_status
      ...
      LEFT JOIN (
        ...
        FROM introductions i
        JOIN job_postings jp2 ON jp2.job_id = i.job_id
        GROUP BY jp2.employer
      ) intro_count ON LOWER(TRIM(intro_count.employer)) = LOWER(e.name)
      GROUP BY e.id, e.name, e.slug, e.claimed, e.tier, e.created_at, ji_count.cnt, intro_count.cnt
...
export async function getVerificationRequests(status = null) {
  const sql = getDb();
  return sql`
    SELECT vr.*,
           ea.email as account_email, ea.name as account_name,
           ea.work_email, ea.work_email_verified_at,
           e.name as employer_name, e.domain as employer_domain, e.website as employer_website
    FROM employer_verification_requests vr
    JOIN employer_accounts ea ON vr.employer_account_id = ea.id
    JOIN employers e ON vr.employer_id = e.id
    WHERE (${status}::text IS NULL OR vr.status = ${status})
    ORDER BY vr.created_at DESC
  `;
}

export async function approveVerification(requestId, adminEmail, domain = null) {
  const sql = getDb();

  // Get request details
  const requests = await sql`SELECT * FROM employer_verification_requests WHERE id = ${requestId}`;
  ...
  // Update account. Admin approval is a completed, trustworthy signal — if
  // the account only holds a pending (two-phase) claim because the setup
  // wizard wasn't finished, promote it to the real link now.
  await sql`
    UPDATE employer_accounts
    SET verification_status = 'verified', verified_at = NOW(), verified_by = 'admin_approved',
        employer_id = COALESCE(employer_id, pending_employer_id),
        pending_employer_id = NULL, pending_claimed_at = NULL
    WHERE id = ${req.employer_account_id}
  `;
  await sql`
    UPDATE employers SET claimed = TRUE
    WHERE id IN (SELECT employer_id FROM employer_accounts WHERE id = ${req.employer_account_id} AND employer_id IS NOT NULL)
  `;

  // Optionally set company domain for future auto-verifications
  if (domain) {
    await sql`UPDATE employers SET domain = ${domain} WHERE id = ${req.employer_id}`;
  }

  return req;
}

export async function rejectVerification(requestId, adminEmail, notes = null) {
  const sql = getDb();
  ...
  await sql`
    UPDATE employer_verification_requests
    SET status = 'rejected', reviewed_by = ${adminEmail}, reviewed_at = NOW(), notes = ${notes}
    WHERE id = ${requestId}
  `;

  // Update account — unlink from employer (including a pending two-phase claim)
  await sql`
    UPDATE employer_accounts
    SET verification_status = 'rejected', employer_id = NULL,
        pending_employer_id = NULL, pending_claimed_at = NULL
    WHERE id = ${req.employer_account_id}
  `;
...
export async function getTeamMembers(employerId) {
  const sql = getDb();
  return sql`
    SELECT id, email, name, role, verification_status, verified_at, created_at
...
export async function getTeamInvitations(employerId) {
  const sql = getDb();
  return sql`
    SELECT ei.*, ea.name as invited_by_name, ea.email as invited_by_email
    FROM employer_invitations ei
    JOIN employer_accounts ea ON ei.invited_by = ea.id
    WHERE ei.employer_id = ${employerId} AND ei.accepted_at IS NULL AND ei.expires_at > NOW()
    ORDER BY ei.created_at DESC
  `;
}

export async function updateTeamMemberRole(memberId, employerId, newRole) {
  const sql = getDb();
  await sql`
    UPDATE employer_accounts SET role = ${newRole}
    WHERE id = ${memberId} AND employer_id = ${employerId}
  `;
}

export async function removeTeamMember(memberId, employerId) {
  const sql = getDb();
  // Don't allow removing someone who isn't in this employer
  await sql`
    UPDATE employer_accounts
    SET employer_id = NULL, role = 'member', verification_status = 'unverified'
    WHERE id = ${memberId} AND employer_id = ${employerId}
  `;
}

// Admin-side detach: unlike rejectVerification this also covers accounts that
// auto-verified via domain match (no verification request row exists for those).
export async function detachEmployerAccount(accountId, adminEmail) {
  const sql = getDb();
  const rows = await sql`
    SELECT id, employer_id, pending_employer_id FROM employer_accounts WHERE id = ${accountId}
  `;
  if (!rows.length || (!rows[0].employer_id && !rows[0].pending_employer_id)) return null;
  const employerId = rows[0].employer_id;

  await sql`
    UPDATE employer_accounts
    SET employer_id = NULL, pending_employer_id = NULL, pending_claimed_at = NULL,
        role = 'member', verification_status = 'unverified',
  ...
  await sql`
    ...
    SET status = 'rejected', reviewed_by = ${adminEmail}, reviewed_at = NOW(),
        notes = COALESCE(notes, 'Detached by admin')
    WHERE employer_account_id = ${accountId} AND status = 'pending'
  `;
  if (employerId) {
    await sql`
      UPDATE employers SET claimed = FALSE
      WHERE id = ${employerId}
        AND NOT EXISTS (SELECT 1 FROM employer_accounts WHERE employer_id = ${employerId})
    `;
  }
  return { accountId, employerId };
}
