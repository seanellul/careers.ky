...
export async function rejectVerification(requestId, adminEmail, notes = null) {
  ...
  // Update account — unlink from employer
  await sql`
    UPDATE employer_accounts
...
export async function detachEmployerAccount(accountId, adminEmail) {
  const sql = getDb();
  const rows = await sql`SELECT id, employer_id FROM employer_accounts WHERE id = ${accountId}`;
  if (!rows.length || !rows[0].employer_id) return null;
