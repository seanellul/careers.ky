...
export async function searchTalentRanked(params = {}) {
  ...
  const {
    ...
    page = 1,
    pageSize = 20,
    // employers.id of the searching employer — enforces candidate block
    // lists at query level. Null skips block filtering (e.g. internal jobs).
    employerId = null,
    // "Immediate start" filter: notice period of immediate or 1 week (spec 11.2a)
    immediateStart = false,
    // Tier entitlements (lib/entitlements.js): overseas candidates hidden
    // from non-entitled tiers (spec rule 6); salary stripped when not visible
  ...
  const rows = await sql`
    WITH candidate_base AS (
      SELECT c.id, c.education_code, c.experience_code, c.location_code,
             ...
             -- Non-negotiable platform rule: Caymanian -> PR -> RERC ->
             -- Dependant -> Overseas, before any score. Undeclared rank last.
             CASE c.status
               WHEN 'caymanian' THEN 1
               WHEN 'pr' THEN 2
               WHEN 'rerc' THEN 3
               WHEN 'dependant' THEN 4
               WHEN 'overseas' THEN 5
               ELSE 6
             END as status_tier
      FROM candidates c
      WHERE c.profile_type != 'closed'
        ...
        AND (${includeOverseas ? true : null}::boolean IS NOT NULL
             OR c.status IS DISTINCT FROM 'overseas')
    ),
    cisco_score AS (
 ...
 */
export async function matchCandidatesToJob(jobId, employerId = null) {
  const sql = getDb();
