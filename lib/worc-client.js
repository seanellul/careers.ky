...
function mapWORCJob(item) {
  ...
  return {
    ...
    salary_long: item.salaryLong?.trim() || null,
    frequency_of_payment: item.frequencyOfPayment || null,
    kyd_per_annum: item.kydPerAnnum > 0 ? item.kydPerAnnum : null,
    number_of_positions: item.numberOfPositionsAvailable || 1,
    medical_check: !!item.medicalCheck,
    police_check: !!item.policeCheck,
    driving_license: !!item.drivingLicense,
    cover_letter_required: !!item.isCoverLetterRequired,
