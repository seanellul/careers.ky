...
export async function getSession() {
  ...
  const rows = await sql`
    SELECT s.candidate_id, s.employer_account_id, s.expires_at,
           ...
           ea.pending_employer_id, pe.name as pending_employer_name,
    ...
    LEFT JOIN employers pe ON ea.pending_employer_id = pe.id
  ...
  return {
    ...
    // Mid-setup selection only — never treated as the linked company.
    // Used by /employer/setup to let an abandoned flow resume.
    pendingEmployerId: row.pending_employer_id,
    pendingEmployerName: row.pending_employer_name,
...
export async function sendMagicLinkEmail(email, token, type = "candidate") {
  ...
  const subject =
    type === "employer" ? "Sign in to careers.ky — Employer Portal" : "Sign in to careers.ky";
