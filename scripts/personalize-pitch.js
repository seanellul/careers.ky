...
const SEGMENTS = {
  Legal: {
    ...
    headline: (c) =>
      `${c.jobCount} open positions. ${c.hiringVolume * 6} hours of compliance work. One audit away from trouble.`,
  ...
  Finance: {
    ...
    headline: (c) =>
      `Regulatory audit risk. ${c.jobCount} open roles. The 2025 changes just made it worse.`,
    ctaPrimary: "Start Free Trial",
    ctaSecondary: "Schedule a Compliance Review",
    avgSalary: 110000,
    tier: "Enterprise",
  ...
  Hospitality: {
    ...
    headline: (c) =>
      `${c.jobCount} jobs posted. ${c.hiringVolume} work permits needed. One denial = frozen hiring.`,
  ...
  Construction: {
    ...
    headline: (c) =>
      `${c.hiringVolume} work permits pending. A denied one delays your next project by months.`,
    ctaPrimary: "Start Free Trial",
    ctaSecondary: "Schedule a 15-min Demo",
    avgSalary: 55000,
    tier: "Pro",
  ...
  Retail: {
    ...
    headline: (c) =>
      `${c.jobCount} positions to fill. New WORC rules on every one. We automated it.`,
    ctaPrimary: "Start Free Trial",
...
function main() {
  ...
  if (inputIdx === -1) {
    console.log("Usage:");
    console.log(
      '  node personalize-pitch.js --single \'{"name":"Acme","industry":"Legal","jobCount":10}\''
    );
